...
public class Collectible : MonoBehaviour {
	...
	public void OnTriggerEnter (Collider col)		// Handle pick-up event for the collectible
	{
		if (!collected) 		// Error check, should never be true as the GameObject should be disabled once it is collected
		{
			if (col.tag == "Player")		// Check the collided object is a player controlled object (egg, etc)
			{
